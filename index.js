...
function ensureDownloads() {
	...
	for (const url of urlList) {
		...
		if (!videoExists) {
			const mediaCmd = `yt-dlp --js-runtimes "nodejs:${process.execPath}" -f "bestvideo[height<=720][ext=mp4]+bestaudio[ext=m4a]/best[height<=720][ext=mp4]/best" --write-description --write-auto-subs --write-subs --sub-langs en --convert-subs srt -o "${TEMP_FRAMES_DIR}/../temp_download/%(id)s.%(ext)s" "${url}"`;
			try {
		...
		if (!commentExists) {
			const commentCmd = `yt-dlp --js-runtimes "nodejs:${process.execPath}" --write-comments --skip-download -o "${COMMENTS_DIR}/%(id)s.%(ext)s" "${url}"`;
			try {
